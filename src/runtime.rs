...
use tauri_runtime::{
    ...
    window::{
        DetachedWindow, DragDropEvent, PendingWindow, RawWindow, WebviewEvent, WindowEvent,
        WindowId,
    },
};
    ...
    fn handle_message(&mut self, event_loop: &dyn ActiveEventLoop, message: Message<T>) {
        match message {
            ...
            Message::DragDropScriptEvent {
                ...
                if let Some(event) =
                    browser_client::event_from_script_event(&drag_drop_state, event)
                {
            ...
            #[cfg(target_os = "macos")]
            Message::AccessibilityChanged { enabled } => {
                self.set_browsers_accessibility_state(enabled)
            }
