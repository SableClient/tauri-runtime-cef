...
fn calculate_window_center_position(
    ...
    PhysicalPosition::new(
        work_area.position.x
            + ((work_area.size.width as i32 - window_size.width as i32).max(0) / 2),
        work_area.position.y
            + ((work_area.size.height as i32 - window_size.height as i32).max(0) / 2),
    )
}
    ...
    pub(crate) fn handle_window_message(
        ...
        match message {
            ...
            WindowMessage::IsMinimized(tx) => {
                _ = tx.send(Ok(window.is_minimized().unwrap_or(false)))
            }
